					...
					<?php foreach ( wc_get_account_orders_columns() as $column_id => $column_name ) : ?>
						<td class="text-sm woocommerce-orders-table__cell-<?php echo esc_attr( $column_id ); ?> px-3 py-2" data-title="<?php echo esc_attr( $column_name ); ?>">
							...
							<?php elseif ( 'order-date' === $column_id ) : ?>
								<time class="whitespace-nowrap" datetime="<?php echo esc_attr( $order->get_date_created()->date( 'c' ) ); ?>"><?= date_i18n( 'd-m-Y H:i', strtotime( $order->get_date_created() ) ); ?></time>
...
<?php else : ?>
	<?php wc_print_notice( __( 'You have no orders yet', 'pixelone' ), 'notice' ); ?>
<?php endif; ?>
